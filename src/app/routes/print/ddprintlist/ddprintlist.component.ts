...
import { PrintDdprintlistEditComponent } from './edit/edit.component';

@Component({
  selector: 'app-print-ddprintlist',
  templateUrl: './ddprintlist.component.html',
})
export class PrintDdprintlistComponent implements OnInit {
  actionPath = 'PrintManagement/RunsheetPrinterList.aspx';
  searchPath = '/print/GetDDPrintPager';
  @ViewChild('st', { static: false }) st: STComponent;
  columns: STColumn[] = [
    { title: '', index: ['plant', 'workshop', 'supplier', 'route', 'part_type'], type: 'checkbox', exported: false },
    {
      title: '操作',
      buttons: [
        {
          ...
          modal: {
            size: 'xl',
            component: PrintDdprintlistEditComponent,
          },
        },
      ],
    },
    ...
    { title: '打印机名称', index: 'printer_name', sort: true },
  ];
  selectedRows: STData[] = [];
  ...
  getListItems(value: any, type: any): void {
    ...
    if (value && this.q.workshop.toString() !== tmp_data.last_workshop) {
      ...
      if (type === 'supplier_parint') this.q.supplier = [];
      // tslint:disable-next-line: no-eval
      else eval('this.q.' + type + ' =  [];');
    }
  }
  ...
  Delete(): void {
    if (this.selectedRows.length === 0) {
      ...
      this.http
        .post('/print/DDPrintDelete', this.selectedRows)
  ...
  Create() {
    this.modal.create(PrintDdprintlistEditComponent, { record: { add: true } }, { size: 'xl' }).subscribe(res => {
