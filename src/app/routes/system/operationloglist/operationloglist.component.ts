...
@Component({
  selector: 'app-system-operationloglist',
  templateUrl: './operationloglist.component.html',
})
export class SystemOperationloglistComponent implements OnInit {
  actionPath = 'SystemManagement/OperationLogList.aspx';
  searchPath = '/system/GetOperationLogPager';
  @ViewChild('st', { static: false }) st: STComponent;
  columns: STColumn[] = [
    { title: '操作时间', index: 'operation_time', sort: true, type: 'date', dateFormat: `YYYY-MM-DD HH:mm` },
    { title: '操作人', index: 'operator', sort: true },
    { title: '操作内容', index: 'operation_parameter1', sort: true },
    { title: '操作数据', index: 'operation_detail', sort: true },
  ];
  ...
  q: any = {
    page: new PageInfo(),
    sort: new SortInfo(),
    operation_time: [new Date(this.today + ' 00:00:00'), new Date(this.today + ' 23:59:59')],
  };
  data: any[] = [];
  dataAction: any[] = [];
  sub_model = new ItemData();
  sub_action = new ItemData();
  ...
  ngOnInit() {
    ...
    this.http.get(`/system/GetOperationLogSearType?tpe=1`).subscribe(
      res => {
        if (res.successful) {
          this.sub_model.data = res.data;
        }
      },
      (err: any) => this.msg.error('系统异常'),
    );
    this.http.get(`/system/GetOperationLogSearType?tpe=2`).subscribe(
      res => {
        if (res.successful) {
          this.sub_action.data = res.data;
        }
      },
      (err: any) => this.msg.error('系统异常'),
    );
