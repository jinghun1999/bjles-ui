...
export class AreaRouteComponent implements OnInit {
  ...
  columns: STColumn[] = [
    ...
    { title: '配送路线代码', index: 'workshop_name', sort: true },
    { title: '配送路线名称', index: 'workshop_type_name', sort: true },
    { title: '配送路线类型', index: 'workschedule_type_name', sort: true },
  ...
  delete() {
    if (this.selectedRows.length === 0) {
      ...
      this.modalSrv.confirm({
        ...
        nzOnOk: () => {
          this.http.post('/area/postDeleteRoute', this.selectedRows).subscribe((res: any) => {
            if (res.successful) {
