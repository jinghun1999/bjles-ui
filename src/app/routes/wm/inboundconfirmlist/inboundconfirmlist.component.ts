...
import { WmInboundconfirmlistEditComponent } from './edit/edit.component';

@Component({
  selector: 'app-wm-inboundconfirmlist',
  templateUrl: './inboundconfirmlist.component.html',
})
export class WmInboundconfirmlistComponent implements OnInit {
  actionPath = 'WMManagement/InboundConfirmQueryList.aspx';
  searchPath = '/wm/GetInboundConfirmPager';
  @ViewChild('st', { static: false }) st: STComponent;
  columns: STColumn[] = [
    { title: '', index: ['InboundConfirmId'], type: 'checkbox', exported: false },
    {
      title: '操作',
      buttons: [
        {
          ...
          modal: {
            size: 'xl',
            component: WmInboundconfirmlistEditComponent,
          },
        },
      ],
    },
    { title: '入库请求单号', index: 'RequestSheetNo', sort: true },
    { title: '入库确认单号', index: 'ConfirmSheetNo', sort: true },
  ...
  Create() {
    // this.modal.create(WmInboundconfirmlistEditComponent, { record: { add: true } }, { size: 'xl' }).subscribe(res => {
