...
import { WmIssuerequestlistEditComponent } from './edit/edit.component';

@Component({
  selector: 'app-wm-issuerequestlist',
  templateUrl: './issuerequestlist.component.html',
})
export class WmIssuerequestlistComponent implements OnInit {
  actionPath = 'WMManagement/IssueRequestQueryList.aspx';
  searchPath = '/wm/GetIssueRequestPager';
  @ViewChild('st', { static: false }) st: STComponent;
  columns: STColumn[] = [
    { title: '', index: ['IssueRequestId'], type: 'checkbox', exported: false },
    {
      title: '操作',
      buttons: [
        {
          ...
          modal: {
            size: 'xl',
            component: WmIssuerequestlistEditComponent,
          },
        },
      ],
    },
    { title: '出库请求单号', index: 'RequestSheetNo', sort: true },
    { title: '物料单号', index: 'ResourceSheetNo', sort: true },
    { title: '工厂', index: 'PlantId', sort: true },
    { title: '源仓库', index: 'SupplierId', sort: true },
    { title: '目标仓库', index: 'WorkshopId', sort: true },
    { title: '单据类型	', index: 'business_type_name', sort: true },
    ...
    { title: '实际到货时间', index: 'InOutTime', sort: true, type: 'date', dateFormat: `YYYY-MM-DD HH:mm` },
    ...
    { title: '接收人', index: 'modify_user_name', sort: true },
    { title: '是否手工关闭', index: 'IsClosed', sort: true, type: 'yn', yn: { mode: 'text' } },
    { title: '是否锁定', index: 'IsLocked', sort: true, type: 'yn', yn: { mode: 'text' } },
  ...
  sub_ENUMWMRequestSheetStatus = new ItemData();
  ...
  Create() {
    // this.modal.create(WmIssuerequestlistEditComponent, { record: { add: true } }, { size: 'xl' }).subscribe(res => {
